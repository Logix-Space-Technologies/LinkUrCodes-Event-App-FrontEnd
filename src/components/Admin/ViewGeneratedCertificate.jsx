...
const ViewGeneratedCertificate = () => {
    ...
    useEffect(() => {
        const fetchCertificates = async () => {
            try {
                const token = sessionStorage.getItem('admintoken'); // Admin token from sessionStorage
                const event_id = sessionStorage.getItem('eventViewID'); // Event ID from sessionStorage

                const response = await axios.post('http://localhost:8085/api/certificate/view-certificates-user-ByEvent', {
                    event_id
                }, {
                    headers: {
                        'token': token
                ...
                } else if (response.data.status === 'event id is required') {
...
export default ViewGeneratedCertificate;
